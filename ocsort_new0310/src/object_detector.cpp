...
VisualizeResult(const cv::Mat &img,
  ...
  for (int i = 0; i < results.size(); ++i) {
    ...
    int c1 = (colormap[3 * results[i].class_id + 0] * results[i].class_id)%256;
    int c2 = (colormap[3 * results[i].class_id + 1] * results[i].class_id)%256;
    int c3 = (colormap[3 * results[i].class_id + 2] * results[i].class_id)%256;
    printf("id: %d, color: %d, %d, %d, \n", results[i].class_id, c1, c2, c3);

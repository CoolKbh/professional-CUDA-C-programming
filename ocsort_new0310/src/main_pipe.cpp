...
void PredictVideo(const std::string& video_path,
  ...
  auto colormap = PaddleDetection::GenerateColorMap(20); //mark debug
  ...
  while (capture.read(frame)) {
    ...
    for (const auto& item : result) {
      ...
      *rect.ptr<float>(0,0) = float(item.class_id);
      *rect.ptr<float>(0,1) = item.confidence;
      *rect.ptr<float>(0,2) = float(item.rect[0]);
      *rect.ptr<float>(0,3) = float(item.rect[1]);
      *rect.ptr<float>(0,4) = float(item.rect[2]);
      *rect.ptr<float>(0,5) = float(item.rect[3]);
      dets.push_back(rect);
      if (item.rect.size() > 6) {
        is_rbox = true;
    ...
    auto tracker = octracker->get_trackers();
    for (auto rid =0; rid < tracker.rows; rid++) {
      auto rect = tracker.row(rid);
      PaddleDetection::ObjectResult res_item;
      res_item.class_id = *rect.ptr<float>(0,1);
      res_item.confidence = *rect.ptr<float>(0,1);
      res_item.rect = std::vector<int>{int(*rect.ptr<float>(0,2)), int(*rect.ptr<float>(0,3)), int(*rect.ptr<float>(0,4)), int(*rect.ptr<float>(0,5))};
      printf("out res class: %d, conf: %f", res_item.class_id, res_item.confidence);
      out_result.push_back(res_item);
    }
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_video_file.empty() || FLAGS_camera_id != -1) {
    PredictVideo(FLAGS_video_file, &det, octracker, FLAGS_output_dir);
